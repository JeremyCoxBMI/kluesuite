 ...
 *
 * SmithWatermanOriginal semi-global alignment, using no penalty for mismatch beyond edges of sequences
 *
...
public class SmithWatermanOriginal {
    ...
    public SmithWatermanOriginal(String string1, String string2){
...
//        }
    ...
    public PartialAlignment1 partialAlignment(int row, int col){
        ...
        if (row != rows.length() && col != columns.length() )
            ...
            return new PartialAlignment1(table, columns, rows, row, col);
    ...
    public ArrayList<PartialAlignment1> bestAlignments() {
        ...
        for (int k = 2; k < m; k++) {
            if (table.get(k)[n].score == maximum) result.add(new PartialAlignment1(table, columns, rows, k, n));
        }

        for (int k = 2; k < n + 1; k++) {
            if (table.get(m)[k].score == maximum) result.add(new PartialAlignment1(table, columns, rows, m, k));
        }
...
//     public static String prettyTableToString(ArrayList<TableEntry[]> table, String rows, String columns){
//         SuperString stringo = new SuperString();
//         stringo.add( "        |  ");
//         for (int k = 0; k < columns.length(); k++) {
//             stringo.add(" ");
//             stringo.addAndTrim(columns.charAt(k));
//             stringo.add(" |  ");

    static String prettyTableToString(ArrayList<TableEntry[]> table, String rows, String columns){
        SuperString stringo = new SuperString("        | ");
        for (int k = 0; k < columns.length(); k++) {
            stringo.add( " ");
            stringo.add(columns.charAt(k));
            stringo.add("  | ");
        }
        stringo.add("\n");
            ...
            for (int j=0; j < columns.length()+1; j++){
                if (table.get(k)[j] == null){
                    ...
                    if (table.get(k)[j].move == TableEntry.EMPTY) {
                        stringo.add("X");
                    //TIE defaults to RIGHT
//                    }else if (table.get(k)[j].move == TableEntry.TIE){
//                        stringo.add("T");
                    } else if (table.get(k)[j].move == TableEntry.DIAGONAL){
                        stringo.add("*");
                    ...
                    if (table.get(k)[j].score < 0){
                        stringo.add(Integer.toString(table.get(k)[j].score));
                        stringo.add(" | ");
                    }
                    else {
                        stringo.add(" ");
      ...
      public String prettyTableToString(){
        return prettyTableToString(this.table, this.rows, this.columns);
    }
    ...
    public static void main(String[] args) {
        ...
        ArrayList<PartialAlignment1> pas;

//        sw = new SmithWatermanOriginal (testStrings[0], testStrings[1]);
...
//        }
        ...
        for (int k=0; k < testStrings.length-1; k+=2){
            sw = new SmithWatermanOriginal(testStrings[k], testStrings[k+1]);
            sw.printPrettyTable(System.out);
            pas = sw.bestAlignments();
