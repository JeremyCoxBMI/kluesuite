...
import org.cchmc.kluesuite.socketklue.server.response.LongListResponse;
 ...
 */
@AllArgsConstructor
public class GetHandler extends KlueProtoCodexHandler {
    ...
    protected void call(ByteBuf req, final SocketPacketSink sink) {
        ...
        klue.get(key, new KlueCallback<ArrayList<Long>>() {
            @Override
            public void callback(ArrayList<Long> value) {
                sink.sink(new LongListResponse(value));
            }
